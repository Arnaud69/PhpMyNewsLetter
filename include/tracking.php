...
$array_sub_allowed = array('trk','env','keys','map');
...
$row_cnt = get_id_send($cnx,$list_id,$row_config_globale,$date_min,$date_max);
if($row_cnt['CPTID'] > 0){
	$array_stats_tmp = get_stats_send($cnx,$list_id,$row_config_globale,$date_min,$date_max);
	$array_date_stats = get_stats_date_by_list($cnx,$list_id,$row_config_globale);
	include 'include/lib/constantes_stats.php';
	...
	$row = get_stats_send_global_by_list($cnx,$row_config_globale,$list_id,$date_min,$date_max);
	...
	<div class="col-md-12">'
	. $alert_error_date
	. '<header><h4>' . tr("KEY_NUMBERS_ALL_CAMPAIGNS") . ' : ' . $list_name . '</h4></header>
	<div class="row">
		<form method="post" action="' . $_SERVER['PHP_SELF'] . '">
			<div class="col-md-6">
				<ul class="nav nav-pills">
					<li' . ($view=='trk'?' class="active"':'') . '><a href="' . $_SERVER['PHP_SELF'] . '?page=tracking&token=' . $token . '&list_id=' . $list_id .'&view=trk&begin_stats=' . ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . '&end_stats=' . ($date_max!=''?$date_max:$array_date_stats[0]['DT_MAX']) . '">' . tr("TRACKING_TITLE") . '</a></li>
					<li' . ($view=='env'?' class="active"':'') . '><a href="' . $_SERVER['PHP_SELF'] . '?page=tracking&token=' . $token . '&list_id=' . $list_id .'&view=env&begin_stats=' . ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . '&end_stats=' . ($date_max!=''?$date_max:$array_date_stats[0]['DT_MAX']) . '">' . tr("ENVIRONMENT") . '</a></li>
					<li' . ($view=='keys'?' class="active"':'') . '><a href="' . $_SERVER['PHP_SELF'] . '?page=tracking&token=' . $token . '&list_id=' . $list_id .'&view=keys&begin_stats=' . ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . '&end_stats=' . ($date_max!=''?$date_max:$array_date_stats[0]['DT_MAX']) . '">Chiffres clé des campagnes</a>
					...
					<a class="dropdown-toggle" data-toggle="dropdown" href="#">' . tr('GEOLOCALISATION') . '<span class="caret"></span></a>
						<ul class="dropdown-menu">
							<li><a href="' . $_SERVER['PHP_SELF'] . '?page=tracking&token=' . $token . '&list_id=' . $list_id .'&view=map&begin_stats=' . ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . '&end_stats=' . ($date_max!=''?$date_max:$array_date_stats[0]['DT_MAX']) . '&tm=twn">' . tr('BY_TOWN') . '</a></li>
							<li><a href="' . $_SERVER['PHP_SELF'] . '?page=tracking&token=' . $token . '&list_id=' . $list_id .'&view=map&begin_stats=' . ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . '&end_stats=' . ($date_max!=''?$date_max:$array_date_stats[0]['DT_MAX']) . '&tm=ctry">' . tr('BY_COUNTRY') . '</a></li>
						</ul>
					</li>
				</ul>
			</div>
	...
	<tr>
		...
		<td style="text-align:center"><h2><a data-toggle="tooltip" title="' . tr( "TRACKING_BULLE_CTR" ) . '">' . ($CTR>0?'<b>' . $CTR . '</b>':0) . '%</a></h2></td>
	...
	$selector_list = '<div class="form-group">
	<form action="' . $_SERVER['PHP_SELF'] . '" method="post" name="selected_newsletter"><div align="center">
	<br>' . tr("TRACKING_GOTO_LIST") . ' : <select name="list_id" class="selectpicker" data-width="auto">';
	$slList='';
	...
	$selector_list .= $slList
	. "</select>
	<input type='hidden' name='page' value='tracking' />
	<input type='hidden' name='token' value='" . $token . "' />
	&nbsp;<input type='submit' value=' O K ' class='btn btn-primary' /></div>
	</form>
	</div>";
	...
	switch ( $view ) {
		...
		case 'env':
				...
				FROM ' . $row_config_globale['table_tracking'] . ' 
					WHERE subject IN (
						SELECT id_mail 
							...
							AND a.date>"'. ($date_min!=''?$date_min:$array_date_stats[0]['DT_MIN']) . ' 00:00:00" 
			...
			if ($results_stat_ua->rowCount()>0) {
				...
				foreach ($results_stat_ua as $tab) {
					...
					*/
					}elseif(preg_match('/Microsoft Office Outlook 12\.\d+\.\d+|MSOffice 12/iD', $str, $matches)) {
						$tmpDataUa['Outlook 2007']=$tmpDataUa['Outlook 2007']+$tab['data'];
					}elseif(preg_match('/Microsoft Outlook 14\.\d+\.\d+|MSOffice 14/iD', $str, $matches)) {
						$tmpDataUa['Outlook 2010']=$tmpDataUa['Outlook 2010']+$tab['data'];
					}elseif(preg_match('/Microsoft Outlook 15\.\d+\.\d+/iD', $str, $matches)) {
						$tmpDataUa['Outlook 2013']=$tmpDataUa['Outlook 2013']+$tab['data'];
					}elseif(preg_match('/Microsoft Outlook (?:Mail )?16\.\d+\.\d+/iD', $str, $matches)) {
			...
			if ($results_stat_domaines->rowCount()>0&&$total>0) {
				$datadomaines = '';
				@(int)$cptdomaines;
				(int)$totalAffiche=0;
			...
			if ($results_stat_domaines_clk->rowCount()>0&&$total>0) {
				$datadomainesclk = '';
				$cptdomainesclk = '';
				(int)$totalAffiche=0;
			...
			echo '<table class="table table-striped">
				...
				<tr>
					<td><div align="center"><h4>' . tr("CLICKED_DISTINCT_DOMAINS") . '</h4></div><canvas id="PmnlDistctDomain" /></td>
					<td><div align="center"><h4>' . tr("OPENED_BY_DOMAINS") . '</h4></div><canvas id="PmnlCntClkDomain" /></td>
					<td width="50%" colspan="2" rowspan="2" align="center"><h4>' . tr('OPEN_BY_HOURS') . '</h4><canvas id="ClicByHours" style="width:70%;height:300px;"></canvas> </td>
				</tr>
				<tr>
			...
			var mCbrowser = new Chart(PmnlChartBrowser, { type: "pie",data:{ labels:[' . $databrowser . '],datasets: [{ data: [' . $cptbrowser . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
			document.getElementById("PmnlStatsBrowser-legend").innerHTML = mCbrowser.generateLegend();
			var PmnlChartPim = $("#PmnlPim");
			var mPim = new Chart(PmnlPim, { type: "pie",data:{ labels:[' . $dataua . '],datasets: [{ data: [' . $cptua . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
			...
			var mCplatform = new Chart(PmnlChartPlatform, { type: "pie",data:{ labels:[' . $dataplatform . '],datasets: [{ data: [' . $cptplatform . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
			...
			var mCdevicetype = new Chart(PmnlChartDevicetype, { type: "pie",data:{ labels:[' . $datadevicetype . '],datasets: [{ data: [' . $cptdevicetype . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
			...
			var mCDistctDomain = new Chart(PmnlDistctDomain, { type: "pie",data:{ labels:[' . $datadomaines . '],datasets: [{ data: [' . $cptdomaines . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
			...
			var mCCntClkDomain = new Chart(PmnlCntClkDomain, { type: "pie",data:{ labels:[' . $datadomainesclk . '],datasets: [{ data: [' . $cptdomainesclk . '],backgroundColor:[' . $BACKGROUND_STATS_COLORS . ']}]}});
		...
		case 'keys':
			...
			foreach($array_stats_tmp as $row){
				...
				<td><a data-toggle="modal" target="_blank" data-tooltip="tooltip" href="include/view_original.php?id_mail='.$row['id_mail'].'&list_id='.$list_id.'&token='
					.$token.'" title="Voir le mail original" style="color:#5cb85c"><i class="glyphicon glyphicon-file"></i></a> - ';
				if($row_cnt['CPTID']>0){
					echo '<a data-toggle="modal" data-target="#modalPmnl" data-tooltip="tooltip" href="include/tracklinks.php?id_mail='.$row['id_mail'].'&list_id='.$list_id.'&token='
				...
				echo '</td>
				<td>' . $row['cpt'] . '</td>
				<td>' . ($row['TOPEN']!=''?$row['TOPEN']:0) . '</td>
				<td>' . ($global_open!=''?$global_open:0) . '</td>
				<td>' . ($row['CPT_CLICKED']!=''?$row['CPT_CLICKED']:0) . '</td>
				<td><a data-toggle="tooltip" title="'. tr( "TRACKING_BULLE_OPEN_RATE" ) .'">'.($OPENRATE>0?'<b>' . $OPENRATE . '</b>':0) . '%</a></td>
				<td><a data-toggle="tooltip" title="'. tr( "TRACKING_BULLE_CTR" ) . '">' . ($CTR>0?'<b>' . $CTR . '</b>':0) . '%</a></td>
				<td><a data-toggle="tooltip" title="'. tr( "TRACKING_BULLE_ACTR" ) .'">' . ($ACTR>0?'<b>' . $ACTR . '</b>':0) . '%</a></td>
				<td>' . $row['error'] . '</td>
				<td>' . $row['leave'] . '</td>
				</tr>';
			}
		...
		case 'map':
			...
			switch($tm){
				case 'ctry' :
					...
					if ($results_stat_latlong->rowCount()>0) {
						$latlong='';
						$mapData='';
					...
					AmCharts.ready(function() {
						...
						map.addTitle("Localisations des ouvertures par pays", 14, "rgba(54, 162, 235, 1)");
						map.areasSettings = {
				...
				default :
					...
					AmCharts.ready(function() {
						AmCharts.theme = AmCharts.themes.dark;
						map = new AmCharts.AmMap();
						map.addTitle("Localisations des ouvertures par ville", 14, "rgba(54, 162, 235, 1)");
						map.areasSettings = {
							unlistedAreasColor: "#000000",
							unlistedAreasAlpha: 0.2,
							autoZoom: true,
