...
switch ($op) {
	case 'init':
		...
		if (getSubscribersNumbers($cnx, $row_config_globale['table_email'], $list_id)) {
			...
			<form id="mailform" name="mailform" method="post" action="" class="post_message">
			<div class="alert alert-info">' . tr("COMPOSE_AND_PREVIEW") . '.</div>
			<div class="row">
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-3">
			...
			<div class='col-md-2'>
			<div class='content-box fixed' style='min-width:172px;max-width:175px'>
			<h4>" . tr("ACTIONS") . " :</h4>
			<input type='button' value='" . tr("SAVE_THIS_MESSAGE") . "' id='rec' class='form-control btn btn-success btn-sm' />
			<hr>
			<input type='button' value='" . tr("COMPOSE_PREVIEW") . " &gt;&gt;' onclick='Soumettre()' disabled id='send_preview' class='form-control btn btn-success btn-sm' />
			<hr>
			<input type='button' value='Vérifier les liens' id='veriflinks' disabled class='form-control btn btn-success btn-sm' />
			<hr>
			<h4>" . tr("ATTACHMENTS") . " :</h4>
			<div id='pjs'></div>
			<a href='#modalPmnl' data-toggle='modal'>" . tr("ADD_ONE_OR_MORE_ATTACHMENT") . "</a>
			</div>
			</div>
			</div>
			<input type='hidden' id='type' name='format' value='html' />
			...
			<script>
					...
					function veriflinks(){
						var ds=$('#mailform').serialize();
						$('#rsvl').html(\"<br><div class='alert alert-info col-lg-12 text-center'><img src='css/processing.gif' width='80px' alt='Process analyse liens en cours' data-toggle='tooltip' data-placement='auto' title='Analyse des liens en cours' /></div>\");
						$.ajax({
							type: 'POST',
	...
	case "preview":
		...
		echo '<div class="row">
		<div class="col-md-10">
		<form method="get" action="send_preview.php" id="mailform" name="mailform">
			<div class="alert alert-info">' . tr("STEP_SEND_PREVIEW", $newsletter["preview_addr"]) . '.</div>
			Preview : <a href="" onClick="xx(360,598);return false;">Nexus 5 (360x598)</a>, 
				<a href="" onClick="xx(375,667);return false;">iPhone 6 (375x667)</a>, 
				<a href="" onClick="xx(414,736);return false;">iPhone 6 Plus (414x736)</a>, 
				<a href="" onClick="xx(600,960);return false;">Nexus 7 2013 (600x960)</a>, 
				<a href="" onClick="xx(750,1334);return false;">iPhone 6S (750x1334)</a>, 
				<a href="" onClick="xx(768,1024);return false;">iPad 3 (768x1024)</a>, 
				<a href="" onClick="xx(800,1280);return false;">Nexus 10 (800x1280)</a>, 
				<a href="" onClick="xx(750,1334);return false;">iPhone 7/8 (750x1334)</a>, 
				<a href="" onClick="xx(828,1792);return false;">iPhone XR/11 (828x1792)</a>
			<h4>' . tr("COMPOSE_PREVIEW_TITLE") . ' : </h4><h5>' . $subj . '</h5>
			<div class="iframePreview">
				<iframe src="preview.php?list_id=' . $list_id . '&token=' . $token . '" width="100%" height="300px" min-height="300px" frameborder="0" 
					style="border:0;" scrolling="yes" id="_preview"><p>' . tr("ERROR_IFRAME") . '...</p>
				</iframe>
			</div>
			...
			<div class="content-box fixed" style="min-width:172px;max-width:175px">
			<h4>' . tr("ACTIONS") . ' :</h4>
			<input type="button" onClick=\'window.location.href="' . $_SERVER["PHP_SELF"] . '?page=compose&token=' . $token . '&list_id=' . $list_id . '&op=init"\' 
				value="' . tr("COMPOSE_BACK") . '" class="form-control btn btn-success btn-sm" /><hr>
			<input type="submit" value="' . tr("COMPOSE_SEND") . '" class="form-control btn btn-success btn-sm"/><br><div align="center">(Mode PREVIEW)</div><hr>
			<h4>' . tr("ATTACHMENTS") . ' :</h4>
			<div id="pjs"></div>
			<a href="#modalPmnl" data-toggle="modal">' . tr("ADD_ONE_OR_MORE_ATTACHMENT") . '</a>
			</div>
	...
	case "send_preview":
		...
		<div class="col-md-10">';
		if ($error == "") {
			echo "<div class='alert alert-success'>" . tr("PREVIEW_SEND_OK") . ".";
			if( $type_env=='dev' ) {
				echo '<a data-toggle="modal" data-target="#modalPmnl" data-tooltip="tooltip" href="include/view_log.php?list_id=' . $list_id . '&t=smtp&token=' . $token . '" title="Voir le log de l\'envoi"> <i class="glyphicon glyphicon-search"></i></a>';
			}
			echo '</div>';
			if (isset($code_mailtester) && $code_mailtester != '') {
				$id_mail_tester = strstr($code_mailtester , '@', true);
				echo "<div class='alert alert-success'><a href='https://www.mail-tester.com/" . $id_mail_tester . "' target='_blank'>" . tr("CHECK_SPAM_SCORE_MAIL_TESTER") . "</a></div>";
			}
		...
		<h4>" . tr("ACTIONS") . " :</h4>
		<input type='button' value='" . tr("COMPOSE_BACK") . "' onClick=\"parent.location='" . $_SERVER['PHP_SELF'] 
