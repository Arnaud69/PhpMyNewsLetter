<?php
/*
 * Ce script a pour objectif de reprendre un envoi par tâche planifiée qui s'est arrêté en cours
 * Il faut indiquer le numéro de la liste, le numéro de l'envoi concerné, et le numéro du dernier mail inscrit dans les logs.
 */
...
$task_id = 'pmnl2_tywNYZ';
$begin = 2838;

/*
 * NE RIEN TOUCHER CI DESSOUS, AU RISQUE DE GENERER DE GRAVES ERREURS !!!!
 *
 *
 *
 * 
...
/*
 * peu importe l'état de l'envoi, on recherche juste les infos sur la l'envoi initial, scheduled, done ou running !
 */
$detail_task = $cnx->query('SELECT *
	FROM '.$row_config_globale['table_crontab'] .'
		WHERE job_id="'.$task_id.'"')->fetchAll(PDO::FETCH_ASSOC);
if(count($detail_task)==0){
	...
	$cnx->query("UPDATE " . $row_config_globale['table_send_suivi'] . " ( )
			SET total_to_send='" . $total_suscribers."' 
				WHERE list_id='" . $detail_task[0]['list_id'] . "' 
					AND msg_id='" . $detail_task[0]['msg_id'] . "'");
	$dontlog = 0;
	...
	$errstr .= "============================RESTART=========================\r\n";
	$errstr .= date("d M Y") . "\r\n";
	$errstr .= "Restarted at " . date("H:i:s") . "\r\n";
		...
		for ($i = 0; $i < $to_send; $i++) {
			...
			if (!$mail->Send()) {
				...
				$cnx->query("UPDATE ".$row_config_globale['table_email']."
						SET error='Y',long_desc='" . CleanInput($ms_err_info) . "',campaign_id='".$detail_task[0]['msg_id']."'
