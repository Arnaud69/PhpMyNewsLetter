...
if($type_serveur=='dedicated' && $exec_available){
	...
	$path_postsuper=exec('command -v postsuper');
	while($pipe) {
		$line = @fgets($pipe);
					  ...
					  ) 
					{
						if(trim($path_postsuper)!=''&&substr($path_postsuper,0,1)=='/'){
							...
							if($cnx->query("INSERT IGNORE INTO ".$row_config_globale['table_email_deleted']." (id,email,list_id,hash,error,status,type)
									SELECT id,email,list_id,hash,'Y','".(CleanInput($tab_failed))."','hard'
										FROM ".$row_config_globale['table_email']."
											WHERE email = '".(CleanInput($tab_recipient))."'")) {
	...
	if($mails_en_cours>0){
		...
		echo '<div class="row" style="min-height:300px;">
			<div class="col-md-10">
			<header><h4>'.tr("PENDING_MAILS").' :</h4></header>
			...
			<div class='col-md-2'>
				<div class='content-box fixed' id='ssb' style='min-width:184px;'>
					<header><h4>" . tr("ACTION") . " :</h3></header>
					<div align='center'>
