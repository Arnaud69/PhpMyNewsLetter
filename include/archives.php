...
<form action='" . $_SERVER['PHP_SELF'] . "' method='post' name='archive_form'>";
...
if (!empty($msg_id) && empty($action)) {
	$TOSEND=$cnx->query("SELECT COUNT(email) AS cpt_to_send FROM
				".$row_config_globale['table_email']."
					WHERE list_id	= '".$list_id."'
						AND error = 'N'")->fetch();
	$REALLYSENDED=$cnx->query("SELECT cpt AS cp_really_sended FROM 
				".$row_config_globale['table_send']."
					WHERE id_list = '".$list_id."'
						AND id_mail = '" . $msg_id . "'")->fetch();
				...
				$(function(){
					function progresspump(){ 
						$.ajax({
							...
							type: "GET",
							dataType:"json",
							data:"force=true&list_id=' . $list_id . '&token=' . $token . '&step=send&msg_id=' . $msg_id . '&begin=0&sn=' . $to_send . '",
							success:function(rj){
								begin = rj.begin;
								sn = rj.sn;
								step = rj.step;
						...
						setTimeout(progresspump,' . ($timer_ajax*1000) .');
					}progresspump();});
			});
		</script>
		<div id="msg" style="display:none">
			<div class="row">
				<div class="col-md-10">
					<div class="row">
						<div class="col-md-8">
							<h4 id="send_title" class="alert alert-info">' . tr("PROGRESSION_OF_CURRENT_SEND") . '</h4>
							<div class="progress">
						...
						<div class="col-md-4">
							<h4 id="last_send_mails" class="alert alert-info">' . tr("LAST_SEND_MAILS") . '</h4>
				...
				<div class="col-md-2">
					<div class="stats_overview">
						<div class="overview_today">
							<p class="overview_day"><div class="alert alert-success">' . tr("COMPOSE_SENDING"). '</div></p>
							<p class="overview_count"><span class="done">0,00%</span> ' . tr("SENDED"). '</p>
							<p class="overview_type">' . tr("TOTAL_TO_SEND"). ' : <span id="total_to_send"></span></p>
						</div>
						<div class="overview_previous">
							<p class="overview_day"><b>' . tr("CHRONO"). '</b></p>
							<p class="overview_type">' . tr("LAST_TIME_SEND"). ' : <span id="ch_last"></span></p>
						</div>
					</div>
				</div>
