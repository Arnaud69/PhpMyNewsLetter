<?php
if( $action=='purge_mailq' && $page=='manager_mailq' && $exec_available && $is_postsuper_true && $is_mq_true ){
				...
				foreach($current_object as $item){
					if(trim($item['recipients'])!=''){
						$cnx->query("INSERT INTO ".$row_config_globale['table_email_deleted']." (id,email,list_id,hash,error,status,type)
							SELECT id,email,list_id,hash,'Y','" . CleanInput($item['failed']) . "','hard'
								FROM ".$row_config_globale['table_email']."
									WHERE email = '" . CleanInput($item['recipients']) . "'");
						$cnx->query("DELETE FROM ".$row_config_globale['table_email']." WHERE email='" . CleanInput($item['recipients']) . "'");
...
if( $action=='delete_id_from_mailq' && $page=='manager_mailq' && !empty($id_mailq) && $exec_available && $is_postsuper_true ){
	if(trim($path_postsuper)!=''&&substr($path_postsuper,0,1)=='/'){
		$result = exec('sudo '.$path_postsuper.' -d '.$id_mailq);
		$cnx->query("INSERT INTO ".$row_config_globale['table_email_deleted']." (id,email,list_id,hash,error,status,type)
						SELECT id,email,list_id,hash,'Y','" . CleanInput(urldecode($_GET['status'])) . "','hard'
							FROM ".$row_config_globale['table_email']."
								WHERE email = '" . CleanInput(urldecode($_GET['mail'])) . "'");
		$cnx->query("DELETE FROM ".$row_config_globale['table_email']." WHERE email='" . CleanInput(urldecode($_GET['mail'])) . "'");
	} else {
		$alerte_purge_mailq = "<h4 class='alert_error'>".tr("ROOT_TO_FLUSH_MAIL_QUEUE")."</h4>";
	}
}

if( $action=='flush_and_force_mailq' && $page=='manager_mailq' && !empty($id_mailq) && $exec_available && $is_postsuper_true ){
