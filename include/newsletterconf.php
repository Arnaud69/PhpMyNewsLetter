...
echo '
<header><h4>' . tr("NEWSLETTER_SETTINGS") . ' : <i>' . $list_name . '</i>, <a href="#codeHtml">' . tr("CODE_TITLE") . '</a></h4></header>
<div class="row">
	<div class="col-md-10">
...
if(isset($list_id)&&!empty($list_id)) {
	...
	if(empty($action)) {
		...
		<header><h4 id='codeHtml'>" . tr("CODE_TITLE") . "</h4></header>
		...
		echo "</div>
		<script src='" . $PATH . "js/tinymce/tinymce.min.js'></script>
		<script>
		tinymce.init({
...
echo '	</div>
	<div class="col-md-2">
		<div class="content-box fixedBox">
			<header><h4>' . tr("ACTION") . ' :</h4></header>
			<input type="submit" value="' . tr('NEWSLETTER_SAVE_SETTINGS') . '" class="btn btn-success" />
			<input type="hidden" name="list_id" value="' . $list_id . '" />
