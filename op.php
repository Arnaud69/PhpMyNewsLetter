...
if(in_array($op,$op_true)){
	switch($op){
		...
		case 'saveGlobalconfig':
			...
			$apis_available=($_POST['apis_available']!=''?$_POST['apis_available']:0);
			$unsub_validation_sms=(isset($_POST['unsub_validation_sms'])?$_POST['unsub_validation_sms']:0);
			...
			echo $PMNL_VERSION=trim(file_get_contents('VERSION'));
			$configFile =saveConfigFile(@$PMNL_VERSION,$_POST['db_host'],$_POST['db_login'],
				...
				$nb_backup,$key_dkim,@$apis_available,@$api_key);
		...
		break;
		case 'smtp_add':
			$smtp_name	 =(isset($_POST['smtp_name'])?CleanInput($_POST['smtp_name']):'');
			$smtp_url	 =(isset($_POST['smtp_url'])?CleanInput($_POST['smtp_url']):'');
			$smtp_user	 =(isset($_POST['smtp_user'])?CleanInput($_POST['smtp_user']):'');
			$smtp_pass	 =(isset($_POST['smtp_pass'])?CleanInput($_POST['smtp_pass']):'');
			$smtp_port	 =(isset($_POST['smtp_port'])?CleanInput($_POST['smtp_port']):'');
			$smtp_secure =(isset($_POST['smtp_secure'])?CleanInput($_POST['smtp_secure']):'');
			$smtp_limite =(isset($_POST['smtp_limite'])?CleanInput($_POST['smtp_limite']):'');
			if($smtp_limite==0 || $smtp_limite==''){
				$smtp_limite=1800;
			}
			$cpt_already_exist = $cnx->query('SELECT * FROM '.$row_config_globale['table_smtp'].' 
					WHERE smtp_url="'.$smtp_url.'" 
						AND smtp_port="'.$smtp_port.'"
						AND smtp_user="'.$smtp_user.'"
						AND smtp_pass="'.$smtp_pass.'"')->fetch();
		...
		break;
		case 'smtp_maj':
			$smtp_id	 =(isset($_POST['smtp_id'])?CleanInput($_POST['smtp_id']):'');
			$smtp_url	 =(isset($_POST['smtp_url'])?CleanInput($_POST['smtp_url']):'');
			$smtp_user	 =(isset($_POST['smtp_user'])?CleanInput($_POST['smtp_user']):'');
			$smtp_pass	 =(isset($_POST['smtp_pass'])?CleanInput($_POST['smtp_pass']):'');
			$smtp_port	 =(isset($_POST['smtp_port'])?CleanInput($_POST['smtp_port']):'');
